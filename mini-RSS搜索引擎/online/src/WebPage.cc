...
{
	cout << "WebPage()" << endl;
	//_topWords.reserve(TOPK_NUMBER);在线部分不用
...
void WebPage::processDoc(const string & doc, Configuration & config, WordSegmentation & jieba)
{
	...
	cout << "webPage:重组摘要" << endl;

	//summary
	string temp = doc.substr(bpos + docContentHead.size(), epos - bpos);
	istringstream record(_docContent);
	int i = 0;
	string line;
	while (record>>line) {
		_docSummary=_docSummary+line;
		_docSummary=_docSummary+"\n";
		++i;
		if (i == 10)break;
	}
	if(_docId==251||_docId==364){
